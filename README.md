...
Browsers require a user gesture before the AudioContext can play. The first click anywhere on the page enables audio. A small `♪` button in the bottom-right corner pulses while silent and turns solid when active; after the first click it can be used to mute / unmute at any time.
